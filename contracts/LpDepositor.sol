...
import "./interfaces/dextopia/ILpDepositToken.sol";
...
contract LpDepositor is Ownable {
    ...
    // internal accounting to track SOLID fees for VeTopia stakers and TOPIA lockers
    ...
    function setAddresses(
        IDextopiaToken _topia,
        IVeDepositor _vetopia,
        address _dextopiaVoter,
        ...
        _vetopia.approve(address(_feeDistributor), type(uint256).max);
        votingEscrow.setApprovalForAll(_dextopiaVoter, true);
        votingEscrow.setApprovalForAll(address(_vetopia), true);

        renounceOwnership();
    }

    /**
        @dev Ensure SOLID, TOPIA and VeTopia are whitelisted
     */
    function whitelistProtocolTokens() external {
        ...
        if (!solidlyVoter.isWhitelisted(address(VeTopia))) {
            solidlyVoter.whitelist(address(VeTopia), tokenID);
        }
        if (!solidlyVoter.isWhitelisted(address(TOPIA))) {
            solidlyVoter.whitelist(address(TOPIA), tokenID);
        }
    }

    /**
        @notice Get pending SOLID and TOPIA rewards earned by `account`
        @param account Account to query pending rewards for
        @param pools List of pool addresses to query rewards for
        @return pending Array of tuples of (SOLID rewards, TOPIA rewards) for each item in `pool`
     */
        ...
        returns (Amounts[] memory pending)
    {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (total > 0) {
                ...
                delta -= delta * 15 / 100;
                integral.dyst += 1e18 * delta / total;
                integral.topia += 1e18 * (delta * 10000 / 42069) / total;
            }

            Amounts storage integralFor = rewardIntegralFor[account][pool];
            if (integralFor.dyst < integral.dyst) {
                pending[i].dyst += balance * (integral.dyst - integralFor.dyst) / 1e18;
                pending[i].topia += balance * (integral.topia - integralFor.topia) / 1e18;
            }
        }
        return pending;
    }
    ...
    /**
        @notice Claim SOLID and TOPIA rewards earned from depositing LP tokens
        @dev An additional 5% of TOPIA is also minted for `StakingRewards`
    ...
    function getReward(address[] calldata pools) external {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            uint256 balance = userBalances[msg.sender][pool];
            _updateIntegrals(msg.sender, pool, gauge, balance, total);
            claims.dyst += claimable[msg.sender][pool].dyst;
            claims.topia += claimable[msg.sender][pool].topia;
            delete claimable[msg.sender][pool];
        }
        if (claims.dyst > 0) {
            SOLID.transfer(msg.sender, claims.dyst);
            emit RewardPaid(msg.sender, address(SOLID), claims.dyst);
        }
        if (claims.topia > 0) {
            TOPIA.mint(msg.sender, claims.topia * 99/100);
            TOPIA.mint(treasary,claims.topia * 1/100);
            emit RewardPaid(msg.sender, address(TOPIA), claims.topia);
            // mint an extra 5% for VeTopia stakers
            TOPIA.mint(address(stakingRewards), claims.topia * 100 / 95 - claims.topia);
            emit RewardPaid(address(stakingRewards), address(TOPIA), claims.topia * 100 / 95 - claims.topia);
        }
    }
    ...
    /**
        ...
        @dev This method is unguarded, anyone can claim any reward at any time.
             Claimed tokens are streamed to TOPIA lockers starting at the beginning
    ...
    function claimLockerRewards(
        ...
        if (bribeRewards.length > 0) {
            ...
            for (uint i = 0; i < bribeRewards.length; i++) {
                ...
                if (reward == SOLID) {
                    ...
                    // that will be converted to VeTopia prior to distribution
        ...
        if (amount > 0) {
            // lock 5% of earned SOLID and distribute VeTopia to TOPIA lockers
            uint256 lockAmount = amount / 3;
            VeTopia.depositTokens(lockAmount);
            IFeeDistributor(distributor).depositFee(address(VeTopia), lockAmount);

            // distribute 10% of earned SOLID to VeTopia stakers
            amount -= lockAmount;
    ...
    function whitelist(address token) external returns (bool) {
        ...
        require(votingEscrow.balanceOfNFT(tokenID) > solidlyVoter.listingFee(), "Not enough veSOLID");
    ...
    function onERC721Received(
        ...
        // VeTopiaDepositor transfers the NFT to this contract so this callback is required
        require(_operator == address(VeTopia));
    ...
    function _updateIntegrals(
        ...
        if (user != address(0)) {
            ...
            if (integralFor.dyst < integral.dyst) {
                ...
                claims.dyst += balance * (integral.dyst - integralFor.dyst) / 1e18;
                claims.topia += balance * (integral.topia - integralFor.topia) / 1e18;
