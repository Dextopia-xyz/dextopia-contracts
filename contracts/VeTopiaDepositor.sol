...
import "./interfaces/dextopia/IDextopiaVoter.sol";


contract VeTopiaDepositor is IERC20, Ownable {

    string public constant name = "VeTopia: Tokenized veDyst";
    string public constant symbol = "VeTopia";
    ...
    IDextopiaVoter public topiaVoter;
    ...
    function setAddresses(
        ...
        IDextopiaVoter _dextopiaVoter,
        IFeeDistributor _feeDistributor
    ) external onlyOwner {
        lpDepositor = _lpDepositor;
        topiaVoter = _dextopiaVoter;
        feeDistributor = _feeDistributor;

        // approve fee distributor to transfer this token (for distributing VeTopia)
    ...
    /**
        @notice Deposit SOLID tokens and mint VeTopia
    ...
    function depositTokens(uint256 _amount) external returns (bool) {
        ...
        votingEscrow.increaseAmount(tokenID, _amount);
    ...
    /**
        ...
        @dev Lock times are also extended each time new VeTopia is minted.
    ...
    function extendLockTime() public {
        ...
        if (maxUnlock > unlockTime) {
            votingEscrow.increaseUnlockTime(tokenID, MAX_LOCK_TIME);
    ...
    /**
        ...
        @dev This function is unguarded, anyone can call to claim at any time.
             The new veSOLID is represented by newly minted VeTopia, which is
             then sent to `FeeDistributor` and streamed to TOPIA lockers starting
